...
__install_required_packages() {
  ...
  apt-get update
  apt-get install -yq --no-install-suggests --no-install-recommends build-essential pkg-config curl libssl-dev bsdmainutils patchelf
...
__install_lz4() {
  ...
  # build
  cd "${BUILD_DIR}"
    gmake -w -j8 clean TARGET_OS=linux
    gmake -w -j8 install TARGET_OS=linux \
